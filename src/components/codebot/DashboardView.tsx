...
const COMPLETE_CAPABILITIES: CapabilityItem[] = [
  {
    text: '44 Tools (Executable)',
    detail: '14 Core + 25 Lazy + 5 Flag-gated, all runnable',
  },
  {
    text: 'Agentic Query Loop',
    detail: 'Think→Act→Observe cycle, max 10 iterations',
  },
  {
    text: '3 Multi-Agent Engines',
    detail: 'Coordinator + Swarm + Teammate, all SSE streaming',
  },
  {
    text: 'Session CRUD',
    detail: 'Create / Read / Update / Delete + rename + auto-title',
  },
  {
  ...
  },
  {
    ...
    detail: 'Snip + Auto + Responsive + Micro',
  },
  {
  ...
  },
  {
    ...
    detail: '188 free models across 6 categories',
  },
  {
  ...
  },
  {
    ...
    detail: 'Interactive / KAIROS / Plan / Voice / ...',
  },
  {
    text: 'Rich Content Renderer',
    detail: 'Mermaid / code highlight / GFM tables',
  },
];
                ...
                <div className="bg-gradient-to-b from-emerald-500/5 to-emerald-500/0 p-4 pb-6">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {[
                      ...
                      <div
                        key={i}
                        className="flex items-center gap-1.5 rounded-md bg-muted/50 px-2.5 py-1.5 text-[11px] text-muted-foreground"
                      >
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
                {COMPLETE_CAPABILITIES.map((cap, i) => (
                  <motion.div
                    ...
                    className="flex items-center gap-2.5 rounded-lg bg-muted/30 px-3 py-2.5 transition-colors hover:bg-muted/50"
                  >
            ...
            {QUICK_ACTIONS.map((action) => {
              ...
              return (
                ...
                >
                  <Button
                    variant="outline"
                    className="group h-auto w-full flex-col gap-2 bg-card/50 py-4 hover:bg-accent"
          ...
          <Card className="border-border/50 bg-card/50">
            <CardContent className="p-4">
              <div className="space-y-1.5">
                {[
                  { emoji: '🤖', text: 'Multi-Agent System — Coordinator/Swarm/Teammate 3条路线全部可运行' },
                  { emoji: '🔧', text: '44个工具全部可执行 — Agent自主调用Bash/File/Search/Web' },
                  { emoji: '📋', text: '会话完整CRUD — 内联重命名/两步确认删除/自动标题' },
                  { emoji: '⚡', text: 'Agentic Status — 实时显示 Thinking/Executing/Generating 状态' },
                  { emoji: '🛑', text: 'AbortController — 真正终止SSE流，非仅设flag' },
                  { emoji: '📊', text: '实时流式速度 — chars/sec 速度指标显示' },
                  { emoji: '🎯', text: 'WelcomeState V3 — 工具执行/思考模式/代码生成能力卡片' },
                  { emoji: '✨', text: 'Premium输入区 — 翡翠辉光/字符计数/快捷键提示' },
                  { emoji: '🔄', text: 'SSE协议增强 — loop_iteration/压缩状态/增强meta' },
                  { emoji: '🧠', text: 'Token微压缩 — FileEdit后精确删除旧FileRead结果' },
                ].map((item, i) => (
                  <div
                    key={i}
