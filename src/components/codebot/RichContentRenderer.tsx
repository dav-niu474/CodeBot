...
function MermaidDiagram({ code }: { code: string }) {
  ...
  if (!svg) {
    return (
      <div className="my-2 rounded-lg border border-border/50 bg-zinc-800/30 p-6 flex items-center justify-center">
  ...
  return (
    <div
      className="my-2 overflow-x-auto rounded-lg border border-border/50 bg-zinc-900/50 p-4"
      ref={containerRef}
...
export function RichContentRenderer({ content, isStreaming }: RichContentRendererProps) {
  ...
  const components: Components = {
    code({ className, children, node, ...props }) {
      ...
      return (
        <code
          className="rounded border border-zinc-700/50 bg-zinc-800/60 px-1.5 py-0.5 font-mono text-[0.8em] text-emerald-400/90"
          {...props}
        >
          {children}
        </code>
    ...
    thead({ children }) {
      return (
        <thead className="border-b border-border/50 bg-zinc-800/50">{children}</thead>
      );
    },
    th({ children }) {
      return (
